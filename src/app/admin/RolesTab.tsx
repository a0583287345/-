...
const PERMISSION_GROUPS = [
  {
    ...
    permissions: [
      {
        id: 'manage_users',
        label: 'ניהול משתמשים (הוספה, עריכה, סיסמאות)',
      },
      {
        id: 'manage_roles',
        label: 'ניהול תפקידים והרשאות',
      },
      {
        id: 'view_dashboard',
        label: 'צפייה בלוח הבקרה (Dashboard)',
      },
    ],
  },
  {
    ...
    permissions: [
      {
        id: 'donors_view',
        label: 'צפייה ברשימת התורמים',
      },
      {
        id: 'donors_create',
        label: 'הוספת תורם חדש',
      },
      {
        id: 'donors_edit',
        label: 'עריכת פרטי תורם (כללי)',
      },
      {
        id: 'donors_delete',
        label: 'מחיקת תורם',
      ...
      // הרשאות ברמת שדה
      {
        id: 'donors_view_contact',
        label: 'צפייה בפרטי התקשרות (טלפונים, אימייל, כתובת)',
      },
      {
        id: 'donors_edit_contact',
  ...
  },
  {
    ...
    permissions: [
      {
        id: 'donations_view',
        label: 'צפייה ברשימת התרומות',
      },
      {
        id: 'donations_create',
        label: 'הוספת תרומה חדשה',
      },
      {
        id: 'donations_edit',
        label: 'עריכת תרומה קיימת',
      },
      {
        id: 'donations_delete',
        label: 'מחיקת תרומה',
      ...
      {
        id: 'donations_export',
        label: 'ייצוא תרומות לאקסל',
      },
      // הרשאות ברמת שדה
      {
        id: 'donations_view_amount',
        label: 'צפייה בסכומי התרומות (חסיון פיננסי)',
      },
    ],
  },
  {
    ...
    permissions: [
      {
        id: 'documents_view',
        label: 'צפייה במסמכים (קבלות/טפסים)',
      },
      {
        id: 'documents_upload',
        label: 'העלאת מסמכים חדשים',
      },
      {
        id: 'documents_delete',
        label: 'מחיקת מסמכים',
...
const FLAT_PERMISSIONS = PERMISSION_GROUPS.flatMap(
    ...
    <div className="p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-6">
        {roles.map((role) => (
          <div
            key={role.id}
            className="border border-slate-200 rounded-2xl bg-white shadow-sm hover:shadow-md transition overflow-hidden flex flex-col"
          >
              ...
              <div className="flex flex-wrap gap-1.5">
                {FLAT_PERMISSIONS.map((perm) => {
                  ...
                  return (
                    <span
                      key={perm.id}
                      className="px-2 py-1 bg-blue-50 text-blue-700 border border-blue-100 text-[11px] rounded-md font-medium"
                    >
                      {perm.label}
                    </span>
                  );
              ...
              >
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1.5">
                    שם התפקיד (לדוגמה: עורך, רואה חשבון, מנהל)
                  </label>
            ...
            <div className="p-4 border-t border-slate-100 bg-white shrink-0 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  type="submit"
                  className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm flex justify-center items-center gap-2 transition shadow-sm min-w-[120px]"
                >
                  {actionLoading ? (
